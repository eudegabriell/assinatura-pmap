...
const RecipientsPage: NextPageWithLayout = (props: any) => {
  ...
  const title: string = `"` + props?.document?.title + `"` + "Destinatários | Assinatura PMAP";
      ...
      <div className="mt-10 px-6 sm:px-0">
                ...
                <Button
                  ...
                  onClick={() => {
                    formValues.some((r) => r.email && hasEmailError(r))
                      ? toast.error("Por favor, insira um endereço de email válido.", { id: "invalid email" })
                      : setOpen(true);
                  }}
                  disabled={
                    !hasSubscription || 
        ...
        <div className="mt-10 overflow-hidden rounded-md bg-white p-4 shadow sm:p-6">
          <div className="border-b border-gray-200 pb-3 sm:pb-5">
            <h3 className="text-lg font-medium leading-6 text-gray-900 ">Signatário</h3>
            <p className="mt-2 max-w-4xl text-sm text-gray-500">
              {props.document.status !== DocumentStatus.COMPLETED
                ? "As pessoas que assinarão o documento."
                : "As pessoas que assinaram o documento."}
            </p>
          </div>
          <FormProvider {...form}>
            <form
                        ...
                        <div
                          ...
                          {errors?.signers?.[index] ? (
                            <p className="mt-2 text-sm text-red-600" id="email-error">
                              <XMarkIcon className="inline h-5" /> Email Inválido
                            </p>
                          ) : (
                            ""
                          )}
                            ...
                            {item.sendStatus === "SENT" && item.readStatus !== "OPENED" ? (
                              <span id="sent_icon">
                                <span
                                  id="sent_icon"
                                  className="mt-3 inline-block flex-shrink-0 rounded-full bg-yellow-200 px-2 py-0.5 text-xs font-medium text-gray-800 ">
                                  <CheckIcon className="mr-1 inline h-5" /> Enviado
                                </span>
                              </span>
                            ) : null}
                            ...
                            <Tooltip label="Reenviar">
                              <IconButton
                                ...
                                onClick={(event: any) => {
                                  ...
                                  if (confirm("Reenviar esta solicitação de assinatura?")) {
                                    setLoading(true);
                            ...
                            <Tooltip label="Excluir">
                              <IconButton
                                ...
                                onClick={(event: any) => {
                                  event.preventDefault();
                                  event.stopPropagation();
                                  if (confirm("Excluir esta solicitação de assinatura?")) {
