...
const DashboardPage: NextPageWithLayout = (props: any) => {
  ...
  const stats = [
    ...
    {
      name: "Esperando pelo destinatário",
      stat: "0",
      icon: UsersIcon,
  ...
  return (
    <>
      <Head>
        <title>Painel | Assinatura PMAP</title>
      </Head>
      ...
      <div className="py-10 max-sm:px-4">
        ...
        <div
          ...
          <span id="add_document" className="text-gray-500 group-hover:text-neon-700 mt-2 block text-sm font-medium duration-200">
            Adicionar PDF ao documento.
          </span>
        </div>
        <ReactTooltip
          anchorId="add_document"
          place="bottom"
          content="Envie um arquivo PDF."
        />
      </div>
    </>
  );
...
function getStat(name: string, props: any) {
  if (name === "Rascunho") return props.dashboard.drafts;
  if (name === "Esperando pelo destinatário") return props.dashboard.waiting;
  if (name === "Assinados") return props.dashboard.completed;
  return 0;
}
